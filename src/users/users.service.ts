...
export class UsersService {
  ...
  async updatePref(
    ...
    const prefUserId = (
      await this.prisma.prefs.findUniqueOrThrow({
        where: { id: prefId },
        select: { userId: true },
      })
    ).userId;
    if (prefUserId !== reqUserId) {
      throw new UnauthorizedException(
        'Vous ne pouvez pas éditer ces préférences',
      );
    }
    return await this.prisma.prefs.update({
      where: { id: prefId },
      data: body,
    });
  }
  ...
  async deletePref(prefId: number, reqUserId: number): Promise<void> {
    ...
    if (prefUserId !== reqUserId) {
      throw new UnauthorizedException(
        'Vous ne pouvez pas supprimer ces préférences',
      );
    }

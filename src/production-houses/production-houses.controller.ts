...
export class ProductionHousesController {
    ...
    @ApiBadRequestResponse({type: BadRequestException })
    @Get(':id')
    @ApiParam({name: 'id', description: 'id of the production house. Must be a number'})
    ...
    async getOne(@Param('id') id: string, @Query('position') position: string) {
        ...
        try {
            return await this.productionHousesService.getOne(castNumParam('id', id), coordinates);
        } catch(e) {
    ...
    @ApiBearerAuth()
    @ApiParam({name: 'id', description: 'id of the production house. Must be a number'})
    async update(@Param('id') id: string, @Body() productionHouse: UpdateProductionHouseDTO): Promise<ProductionHouse> {
        try {
            return await this.productionHousesService.updateProductionHouse(castNumParam('id', id), productionHouse);
        } catch(e) {
            handleErrorResponse(e, 'id', id);
        }
    }
    ...
    async delete(@Param('id') id: string): Promise<void> {
        try {
            return await this.productionHousesService.deleteProductionHouse(castNumParam('id', id));
        } catch(e) {
            handleErrorResponse(e, 'id', id);
        }
    }
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiParam({name: 'productionHouseId', description: 'id of the production house. Must be a number'})
    @ApiParam({name: 'accessibilityId', description: 'id of the accessibility. Must be a number'})
    async addAccessibility(@Param('productionHouseId') productionHouseId: string, @Param('accessibilityId') accessibilityId: string): Promise<string> {
        let paramsError = [];
        ...
        return await this.productionHousesService.addAccessibility(+productionHouseId, +accessibilityId);
    }
    ...
    @ApiParam({name: 'productionHouseId', description: 'id of the production house. Must be a number'})
    @ApiParam({name: 'accessibilityId', description: 'id of the accessibility. Must be a number'})
    async removeAccessibility(@Param('productionHouseId') productionHouseId: string, @Param('accessibilityId') accessibilityId: string): Promise<void> {
        let paramsError = [];
        if(isNaN(+productionHouseId)) {
            paramsError.push('productionHouseId');
        }
        ...
        return await this.productionHousesService.removeAccessibility(+productionHouseId, +accessibilityId);
    }
}
